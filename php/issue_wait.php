...
<body class="nav-sm">
    ...
    .issue-row-incomplete {
        background-color: #fff4e6 !important; /* Light brownish background */
    }
    
    .issue-row-incomplete:hover {
        background-color: #ffe8cc !important; /* Slightly darker on hover */
    }
    ...
    .issue-row-completed:hover {
        background-color: #ccffcc !important; /* Slightly darker green on hover */
    }
        ...
        function loadIssueHistory(cusID) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    let timeTaken = '';
                    let formattedChkTime = '';
                    
                    if (issueHistory.chk_date && issueHistory.chk_date !== '0000-00-00' && issueHistory.chk_time) {
                        formattedChkTime = formatAMPM(issueHistory.chk_time);
                        
                        if (issueHistory.rDate && issueHistory.rTime) {
                            let startDateTime = new Date(issueHistory.rDate + ' ' + issueHistory.rTime);
                            let endDateTime = new Date(issueHistory.chk_date + ' ' + issueHistory.chk_time);
                            ...
                            timeTaken = `${diffDays} day(s) ${diffHours} hour(s)`;
                        }
                    }
                    
                    // Determine row class based on completion status
                    //let rowClass = (issueHistory.issueStatus !== 'Completed') ? 'issue-row-incomplete' : '';
                    let rowClass = (issueHistory.issueStatus !== 'Completed') ? 'issue-row-incomplete' : 'issue-row-completed';
                    
                    // Determine priority class
                    let priorityClass = '';
                    if (issueHistory.priorityLevel === 'High') {
                        priorityClass = 'priority-high';
                    } else if (issueHistory.priorityLevel === 'Medium') {
                        priorityClass = 'priority-medium';
                    } else if (issueHistory.priorityLevel === 'Low') {
                        priorityClass = 'priority-low';
                    }
                    priorityDisplay = `<span class="badge ${priorityClass}">${issueHistory.priorityLevel}</span>`;
                            ...
                            $('.tdataIssues').append(`
                                <tr  class="${rowClass}">
                                    ...
                                    <td>${issueHistory.rDate} ${formatAMPM(issueHistory.rTime)}</td>
                                    
                                    <td>${issueHistory.chk_date && issueHistory.chk_date !== '0000-00-00' ? issueHistory.chk_date + ' ' + formattedChkTime : ''}</td>
                            <td>${timeTaken}</td>
                                    ...
                                    <td>${issueHistory.AgentComment || ''}</td>
                                    <td>${issueHistory.DeveloperComment || ''}</td>
