...
$cus_id = mysql_real_escape_string($_GET['cus_id']);
$reportPrnt = 'itm_cat';
$reportslect='cate';
...
$catQry = '';
if($categoryID!=''){
	$catQry = "AND itemtable.CatID = '$categoryID'";
}
...
<!--
    ...
    <div id="ajaxRespon" style="width:98%; ">                
  		 <div id="remove_TTb" class="findexcel" style="margin-left:34px;"> 
			...
			$sql_cusDet = mysql_query("SELECT ID FROM custtable WHERE CustomerID = '$cus_id'");
			$cusDet = mysql_fetch_array($sql_cusDet);
			$cusTb = $cusDet['ID'];
				// category option -----------------------
				$sql_c_u_p = mysql_query("SELECT CatID FROM itemtable Group by CatID ORDER by `CatID` ASC ");
		  ...
		  while($c_u_p = mysql_fetch_array($sql_c_u_p))
		 {
			...
			$cusID_Qry = ($cus_id != 'View All')?"AND invoice.cusTbID = '$cusTb'":"";
				 if($sales_id != 'View All' )
				  ...
				  { 
					...
					//echo $qry_inv;
					 ...
					 $row_count=mysql_num_rows($qry_due);
			 ...
			 if($row_count >0)
			 {
			  ...
			  while($due = mysql_fetch_array($qry_due)){
				  ...
				  if($sales_id != 'View All' )
				  {
					 ...
					 $qry_inv_fre=mysql_query($qry_fre);
				  $free_qty=mysql_fetch_array($qry_inv_fre);
				  $tot_fre +=$free_qty[0];
				  $sum_fre += $free_qty[0];
				  
						$tot_qty=$tot_qty+$due[0];
						$val=$due[2] ;
						$tot_price= $due[2] / $due[0];
						$tot_val=$tot_val + $val;
						...
						$qry_inv= mysql_query("SELECT `invitem`.ID FROM `invitem` JOIN invoice ON `invitem`.`InvID` =invoice.ID
						...
						$noINV= mysql_num_rows($qry_inv);
						
						$qry_cus= mysql_query("SELECT invitem.ID FROM `invitem` JOIN invoice ON `invitem`.`InvID` =invoice.ID 
						...
						else
						{
							 echo'
								 <tr>
									...
									<td style="text-align:right;" >'.number_format($due[2] / $due[0],2).'</td>
									<td style="text-align:right;">'.number_format($free_qty[0],2).'</td>
									<td style="text-align:right; " >'.number_format($due[2] / ($due[0]+$free_qty[0]),2).'</td>
