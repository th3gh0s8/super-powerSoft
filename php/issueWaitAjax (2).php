...
if ($action == 'loadIssueHistory') {
    ...
    $sql_loadHistory = $mysqli->query("SELECT * FROM issue_tb WHERE cusID = '$cusId' ORDER BY ID DESC");
    $historyArray = [];
    $today = new DateTime();
...
if ($action == 'loadDataByID') {
    ...
    // Issue main details - ORDER BY ID DESC
    $sql_issueCmnt = $mysqli->query("SELECT * FROM issue_tb WHERE ID = '$getByID' ORDER BY ID DESC");
    $rowDet = $sql_issueCmnt ? $sql_issueCmnt->fetch_assoc() : [];
