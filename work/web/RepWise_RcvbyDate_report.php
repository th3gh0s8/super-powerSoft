...
if($user_type == 'Admin'){	
	...
	$sql_Uname11 = $mysqli->query("SELECT salesrep.Name,salesrep.RepID  FROM `sys_users` JOIN salesrep ON sys_users.`repID` = salesrep.RepID 
				WHERE sys_users.`ID` = '$user_id'");
				
		$uNAME = $sql_Uname11->fetch_array();
		
		$issuedItem=$mysqli->query("SELECT * FROM `user_rights` JOIN pages ON `user_rights`.`page_id`= pages.ID WHERE
				 `user_id` ='$user_id' AND `page_title` ='control_Rep_Control' AND `user_rights`.`br_id`='$br_id'");
					$count_issuedItem=$issuedItem->num_rows;
...
//$dateop=$mysqli->query("SELECT `optionPath` FROM `optiontb` WHERE `br_id`='$br_id' AND `opertionName`='sales_rep_recieve' ");
//$dateopDT=$dateop->fetch_array();
//echo $dateopDT[0];
...
.amountBox, .paymntBox{
   ...
   <div id="remove_TTb" class="findexcel">
    <table class="table table-striped table-bordered bootstrap-datatable responsive invoSumryTB" style="margin-top:10px";>
        ...
        <tr>
			<th colspan="10">Sales ID / Name : <?php echo $sales_id.' / '.$sl_name ?></th>
		...
		while($Findcus = $sqlFindcus->fetch_array()){
			...
			/*$sqlCusNm = $mysqli->query("SELECT cusName FROM `custtable` WHERE `CustomerID`= '$Findcus[1]'");
			$CusNm = $sqlCusNm->fetch_array(); */
				
			/*$sqlFindRcvRW= $mysqli->query("SELECT *
	...
	/*	echo'<tr>
			...
			if($invage['RepID'] == 'Rtn Chq' ||  $invage['RepID'] == 'Rtn Chgs' ){
					...
					$qry_rep=$mysqli->query("SELECT salesrep.Name FROM `repcustomize` JOIN salesrep 
					ON `repcustomize`.`rep_id` =salesrep.ID  WHERE `cus_id`='$cusID' ");
					$rst_rep=$qry_rep->fetch_array();
					$repname=$rst_rep[0];
		...
		{
		 echo'<tr>
			<th  colspan="10" style="background-color:#DFC36B; text-align:right;" height=20px >      </td>
