...
<div class="row">-->
    ...
    <div id="ajaxRespon" class=" col-sm-12"  style="width:100%; margin-top:-30px;">    
		...
		$qry_sale=$mysqli->query("SELECT SUM(`Quantity`) AS a, SUM(invitem.Sprice * `Quantity` * diff_inv) avgSprice, `itm_code`,`itm_id`,`qty`,itm_id, `Date`, invitem.diff_inv, invitem.Sprice FROM `invitem` RIGHT OUTER JOIN `br_stock` ON `invitem`.`itemID` = br_stock.itm_id AND `invitem`.`br_id` = br_stock.br_id WHERE `br_stock`.`br_id`='$br_id' 
		AND `invitem`.Date BETWEEN '$from_d' AND '$to_d' GROUP BY itm_id ORDER BY `a` DESC");



				$n=0;		$totalAmnt = 0;
			  while($rst_sale=$qry_sale->fetch_array()){
				  $n++;
				  ...
				  $qry_itmTB=$mysqli->query("SELECT Description,MinSale,CatID,Sprice, XWord FROM `itemtable`   WHERE ID='".$itm_id."'");
				  $itmTB=$qry_itmTB->fetch_array();
			...
			$qry_sale2=$mysqli->query("SELECT `itm_code`,`itm_id`,`qty`, invitem.diff_inv FROM `br_stock` WHERE `br_stock`.`br_id`='$br_id' AND
			 itm_id NOT IN (SELECT `itemID` FROM `invitem` WHERE `invitem`.Date BETWEEN '$from_d' AND '$to_d' AND `br_id`='$br_id') ");
			
		
						
			  while($rst_sale2=$qry_sale2->fetch_array()){
				  $n++;
				  //echo $n;
				  ...
				  $qry_itmTB2=$mysqli->query("SELECT Description,MinSale,CatID,Sprice, XWord FROM `itemtable`   WHERE ID='".$itm_id."'");
				  $itmTB2=$qry_itmTB2->fetch_array();
					 	$tt = $itmTB2['Sprice']*0;
