<?php
	...
	mysql_query("UPDATE `invitem` SET `diff_inv` = '1' WHERE `diff_inv` = '0' ");
	mysql_query("UPDATE `invitem` SET `diff_inv` = '1' WHERE `UnitNo` = '1' ");		
	...
	if($sales_id == 'View All'){
		$sql_CAT = mysql_query("SELECT CatID, itemtable.`XWord`, itemtable.`Vendor`, invitem.`InvNo`
  		...
  		<div class="main_container">
			...
			<div class="right_col" role="main">
						...
						<div class="form-group" style="margin-bottom:; margin-left:;width:100%;">
							...
							<div id="remove_TTb" class="findexcel">
								<table class="table table-striped table-bordered bootstrap-datatable  responsive invoSumryTB" style="margin-top:30px";>
									...
									<?php
    								echo'<tbody>';
										$sql_GDTT = mysql_query("SELECT (CostPrice*Quantity)/diff_inv,Sprice*Quantity,Quantity
										...
										while($GDTT = mysql_fetch_array($sql_GDTT)){
											if($GDTT[2] > 0){
												$ps += $GDTT[1];
												$pc += $GDTT[0];
											}else{
										...
										while($CAT = mysql_fetch_row($sql_CAT)){
											...
											$sql_GnL = mysql_query("SELECT invitem.`ItemNo`, invitem.`Description`, invitem.itemID
											...
											$RWGnL = mysql_num_rows($sql_GnL);
											$RwGnL = $RWGnL + 1 ;
												$getVendor = mysql_query("SELECT * FROM `vendor` WHERE br_id = '$br_id' 
																		  AND ID = '$CAT[2]'");
												$venRow = mysql_fetch_array($getVendor);
											...
											while($GnL = mysql_fetch_array($sql_GnL)){
												$sql_GnLd = mysql_query("SELECT SUM(`Quantity`), SUM(CostPrice*Quantity), SUM(invitem.Sprice*diff_inv*Quantity) , itemtable.`XWord`, itemtable.`Vendor`,invoice.RepID
													FROM `invitem` JOIN itemtable 
													ON `invitem`.itemID = itemtable.ID
													JOIN `invoice` ON `invitem`.`invID`  = `invoice`.`ID`
													WHERE invitem.`Date` BETWEEN '$from_dd'
												...
												$GnLd = mysql_fetch_array($sql_GnLd);
				...
				$itesmVal = $sql_items1->fetch_array();
						 ...
						 $costVal = $itesmVal[0];
						 $salesVal = $itesmVal[1]; 
						 $percentage = number_format(($costVal != 0) ? (($salesVal-$costVal) / $costVal) * 100 : 0,2);
												
												
												
												$sql_brstock = mysql_query("SELECT qty FROM br_stock WHERE itm_id = '$GnL[2]' AND br_id = '$br_id'");
												$brstock = mysql_fetch_array($sql_brstock);
												...
												if($QTY > 0){
													...
													$ItmPer   = ($Tcst == 0) ? 0:100*$balnc/$Tcst ;
													$TTsl    += $Tsl;
													$TTcst   += $Tcst;
													$Tbalnc  += $balnc;
													$Tbalnc2 += $balnc;
													$TItmPer += $ItmPer;
													$subPer   = 100*$Tbalnc/$tt;
												}else{
													$clr      = 'red';
													$SL = 0;
													$CST = 0;
								...
								// }
								$repName = mysql_query("SELECT Name FROM salesrep WHERE RepID = '".$GnLd['RepID']."'");
								//echo 'WWWWW'.("SELECT Name FROM salesrep WHERE RepID = '".$GnLd['RepID']."'");
								$RName = mysql_fetch_array($repName);
								$RName = $RName[0];
											            // <td>'.$SubCat.'</td>
											            // <td>'.$vendorName.'</td>
											            // <td>'.$CAT[3].'</td>
											...
											$GDCOSTT += $TTcst;
											$ad = 0;
											if($GDSLTT > 0 || $GDCOSTT > 0){
												$ad       = 100*($GDSLTT-$GDCOSTT)/$GDCOSTT ;
											}
											$final   += $sub2Per;
										}
									echo'</tbody>';
